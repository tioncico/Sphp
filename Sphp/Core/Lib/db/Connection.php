...
abstract class Connection
{
    ...
    public static function instance($config = [], $name = false)
    {
        ...
        if (true === $name || !isset(self::$instance[$name])) {
            ...
            $class = false !== strpos($options['type'], '\\') ? $options['type'] : '\\Sphp\\Core\\Lib\\db\\connector\\' . ucwords($options['type']);
            // 记录初始化信息
    ...
    public function getBuilderClass()
    {
        if (!empty($this->builderClassName)) {
            ...
            return $this->getConfig('builder') ?: '\\Sphp\\Core\\Lib\\db\\builder\\' . ucfirst($this->getConfig('type'));
        }
    }
    ...
    public function find(Query $query)
    {
        ...
        $cacheHandler = \Sphp\Core\Db::getCacheHandler();
    ...
    {
        if (empty($config)) {
            $config = \Sphp\Core\Db::getConfig();
        } elseif (is_string($config) && false === strpos($config, '/')) {
            // 支持读取配置参数
            $config = \Sphp\Core\Db::getConfig($config);
        }
